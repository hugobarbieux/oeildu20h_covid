...
#nombre de personnes ayant reçu au moins une dose, par classes d’age, arrete a la dernière date disponible, echelle departementale
...
#livraisons départementales réalisées et prévisionnelles par semaine, en UCD (flacons) et doses comptabilisées comme la somme des différents flux départementaux
...
# Donnees concernant les deuxiemes doses, par departement
...
dt9$clage_vacsi <- clagedt9 #associer la valeur de ce vecteur à une nouvelle colonne nommee clage_vacsi
...
## Carte des livraisons de vaccins
...
map_france <- tm_shape(data) +
  ...
  tm_layout("Doses livrées - total", title.size = 1.8, bg.color="#e6e6e6", legend.title.size=1.2, fontfamily = "Brown",
...
## Courbes de la campagne de vaccination dose 1 (valeurs absolues)
...
p <- ggplot(vacsi_a_fra_2doses, aes(x = jour, y = cum_dose1, colour=as.character(clage_vacsi)))+
  ...
  scale_y_continuous(limit = c(0,1280000), labels=function(x) format(x, big.mark = " ", scientific = FALSE)) +
...
## Courbes de la campagne de vaccination dose 2 (valeurs absolues)
...
p <- ggplot(df, aes(x = jour, y = cum_dose2, colour = as.character(clage_vacsi))) +
  ...
  geom_hline(yintercept = 0, size = 1.2, colour="#767676") +
  scale_colour_manual(values = c('#450256','#F9E721','#21908D','#5AC865'),
                      labels = c("40-59 ans", "60-74 ans", "75 ans et plus", "EHPAD")) +
  bbc_style() +
  labs(title="COVID-19 - Vaccination",
  ...
  theme(plot.title = element_text(size = 15, color = "#4e4d47", family = "Brown",
        ...
        panel.grid.major.x = element_line(color = "#d9dddc")) +
  scale_x_date(date_labels = "%d %b", date_breaks = "3 days") +
  scale_y_continuous(limit = c(0,160000), labels=function(x) format(x, big.mark = " ", scientific = FALSE)) +
  geom_vline(xintercept = as.numeric(df$jour[dose_2_vline]),
             col = "black", lwd = 0.5) +
  geom_label(aes(x = as.Date(c("2021-02-06")), y = 140000, label = "Introduction du\nvaccin AstraZeneca"), 
                           hjust = "right", 
                           vjust = 0.5, 
                           colour = "#555555", 
                           fill = "transparent", 
...
Alignement des courbes des doses 1 et 2 pour comparer le suivi des injections
...
#Reconstituer la dataframe dans une nouvelle variable et conserver seulement certaines dates pour "zoomer" dans la période intéressante
...
p <- ggplot(vacsi_a_fra_2doses_bis, aes(x = jour, colour=as.character(clage_vacsi)))+
  ...
  theme(plot.title = element_text(size = 15, color = "#4e4d47", family = "Brown",
        ...
        panel.grid.major.x = element_line(color = "#d9dddc")) +
  scale_x_date(date_labels = "%d %b", date_breaks = "3 days") +
  scale_y_continuous(limit = c(0,400000), labels=function(x) format(x, big.mark = " ", scientific = FALSE)) +
  #geom_vline(xintercept = as.numeric(vacsi_a_fra_2doses$jour[dose_1_vline]),
             #col = "black", lwd = 0.5) +
  geom_label(aes(x = as.Date(c("2021-01-06")), y = 300000, label = "Les trajectoires des\nsecondes vaccinations\n(en pointillés)\nont ete ramenees\nartificiellement 21 jours\nplus tot pour suivre\nles premieres series\nd'injections"), 
                           hjust = "left", 
                           vjust = 0.5, 
                           colour = "#696969", 
                           fill = "transparent", 
...
*Taux d'incidence = nombre de cas positifs pour 100 000 personnes par semaine.
