...
### Chiffres de livraisons, __par 100 000 habitants__, tous vaccins confondus.
...
data_map2 <- FR_departements %>%
  left_join(livraisonsdf, by = "dep") %>%
  arrange(desc(nb_doses_total)) %>%
  mutate(doses_par_habitant = round((nb_doses_total/pop_total)*100000, digits = 2)) %>%
  mutate(text = paste0(nom, " : ", doses_par_habitant, " doses/100 000 hab.")) %>%
...
map_france2 <- tm_shape(data_map2) +
  ...
  tm_credits("Source : data.gouv.fr",
             col = "grey10",
             position = c("right", "bottom")) +
  tm_layout("Doses livrées, par 100 000 hab.- total",
            title.size = 1.8,
            bg.color="#e6e6e6",
            legend.title.size=1.2,
            fontfamily = "Brown",
  ...
  tm_bubbles(id = "text",
             ...
             style="equal",
             title.col="Nombre de doses, 100k hab.",
             legend.size.show = FALSE)
map_france2
```
...
> __Depuis le début de la campagne de vaccination en France, 2 254 688 premieres injections de vaccin (soit 3,4% de la population totale et 4,3% de la population majeure) et 647 173 deuxiemes injections ont ete realisees _(donnees provisoires en attente de consolidation)___. (Communique de la DGS - 14 fevrier 2021)
...
# Create dataframe
data <- data.frame(
  category=c("Personnel de sante\n+ 50 ans", "Autre personnel de sante", "Reste de la population"),
  count=c(535630,930854,67422241)
)
