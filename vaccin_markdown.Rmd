...
public_prior <- soignants_df %>%
  ...
  left_join(livraisonsdf, by = "dep") %>%
  mutate(`doses/hab prioritaire` = round((nb_doses_total/public)*100, digits = 0)) %>%
  mutate(text = paste0(nom, " : ", `doses/hab prioritaire`, " doses pour 100 habitants prioritaires"))
...
mypalette <- c("#f0f9e8","#eb0032")

map_france_prior <- tm_shape(map_publicprior_df) +
  tm_polygons(id = "text",
              col = "doses/hab prioritaire",
              palette = mypalette,
              alpha = 0.8,
              contrast = 1.2,
              border.col = "#e6e6e6") +
  ...
  #            alpha = 0.3,
  #            border.col = "#e6e6e6") +
  tm_layout(main.title = "Doses livrées pour\n100 habitants prioritaires",
            title.size = 1,
            bg.color="#e6e6e6",
...
#### Répartition des doses de vaccins Pfizer pour 100 habitants prioritaires
...
data_map_pfizer <- FR_departements %>%
  ...
  left_join(soignants_df, by = "dep") %>%
  mutate(public = (`75_plus` + soignants_prioritaires)) %>%
  mutate(ratio_pfizer = round((nb_doses_total/public)*100, digits = 0)) %>%
  arrange(desc(nb_doses_total)) %>%
  mutate(text = paste0(nom, " : ", ratio_pfizer, " doses Pfizer/100 hab. prioritaires"))
...
map_france_pfizer <- tm_shape(data_map_pfizer) +
  ...
  tm_credits("Source : data.gouv.fr",
             col = "grey10",
             position = c("right", "bottom")) +
  tm_layout("Doses livrées\npour 100 habitants prioritaires",
            title.size = 1,
            bg.color="#e6e6e6",
            legend.title.size=0.95,
            fontfamily = "Brown",
...
map_france_prior <- tm_shape(centres_vax_map_df) +
  ...
  tm_credits("Source : data.gouv.fr",
             col = "grey10",
             position = c("right", "bottom")) +
  tm_layout("Nombre de centres de vaccination\npour 100 000 habitants",
            title.size = 1,
            bg.color="#e6e6e6",
            legend.title.size=0.95,
            fontfamily = "Brown",
...
map_france2 <- tm_shape(data_map2) +
  tm_polygons(id = "text", col = "#b59475",
              alpha = 0.3,
              border.col = "#e6e6e6") +
  tm_credits("Source : data.gouv.fr",
             col = "grey10",
...
p <- ggplot(vacsi_a_fra_2doses, aes(x = jour, y = cum_dose1, colour=as.character(clage_vacsi)))+
  ...
  scale_y_continuous(limit = c(0,1550000), labels=function(x) format(x, big.mark = " ", scientific = FALSE)) +
...
p <- ggplot(df, aes(x = jour, y = cum_dose2, colour = as.character(clage_vacsi))) +
  ...
  theme(plot.title = element_text(size = 15, color = "#4e4d47", family = "Brown",
        ...
        panel.grid.major.x = element_line(color = "#d9dddc")) +
  scale_x_date(date_labels = "%d %b", date_breaks = "3 days") +
  scale_y_continuous(limit = c(0,600000), labels=function(x) format(x, big.mark = " ", scientific = FALSE)) +
  geom_vline(xintercept = as.numeric(df$jour[dose_2_vline]),
             col = "black", lwd = 0.5) +
  geom_label(aes(x = as.Date(c("2021-02-06")), y = 500000, label = "Introduction du\nvaccin AstraZeneca"), 
                           hjust = "right", 
                           vjust = 0.5, 
                           colour = "#555555", 
                           fill = "transparent", 
...
vacsi_a_fra_2doses_bis <- subset(vacsi_a_fra_2doses_bis, vacsi_a_fra_2doses_bis$jour < as.Date("2021-01-28"))
...
p <- ggplot(vacsi_a_fra_2doses_bis, aes(x = jour, colour=as.character(clage_vacsi)))+
  ...
  theme(plot.title = element_text(size = 15, color = "#4e4d47", family = "Brown",
        ...
        panel.grid.major.x = element_line(color = "#d9dddc")) +
  scale_x_date(date_labels = "%d %b", date_breaks = "3 days") +
  scale_y_continuous(limit = c(0,750000), labels=function(x) format(x, big.mark = " ", scientific = FALSE)) +
  #geom_vline(xintercept = as.numeric(vacsi_a_fra_2doses$jour[dose_1_vline]),
             #col = "black", lwd = 0.5) +
  geom_label(aes(x = as.Date(c("2021-01-06")), y = 500000, label = "Les trajectoires des\nsecondes vaccinations\n(en pointilles)\nont ete ramenees\nartificiellement 21 jours\nplus tot pour suivre\nles premieres series\nd'injections"), 
                           hjust = "left", 
                           vjust = 0.5, 
                           colour = "#696969", 
                           fill = "transparent", 
...
p3 <- ggplot(df2, aes(fill=series, y=value, x=str_wrap(noms, 40))) + 
  ...
  theme(plot.title = element_text(size = 15, color = "#4e4d47", family = "Brown",
        ...
        panel.grid.major.x = element_line(color = "#cbcbcb"),
        panel.grid.minor.x = element_blank()) +
  scale_y_continuous(limit = c(0,20), breaks = c(0,5,10,15,20),
                     labels = scales::percent_format(scale = 1, accuracy = 1)) +
  coord_flip()
...
p5 <- ggplot(df4, aes(fill=series, y=value, x=str_wrap(noms, 40))) + 
  ...
  theme(plot.title = element_text(size = 15, color = "#4e4d47", family = "Brown",
        ...
        panel.grid.major.x = element_line(color = "#cbcbcb"),
        panel.grid.minor.x = element_blank()) +
  scale_y_continuous(limit = c(0, 20), breaks = c(0:20),
                     labels = scales::percent_format(scale = 1, accuracy = 1)) +
  coord_flip()
